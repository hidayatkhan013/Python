...
#•	Bitwise XOR of the block and the key 
    ...
    ansk=dec_xor(nibble_quadruple,key_quadruple)
    ansl=dec_swap_last_two(dec_byte_to_nibbles(ansk))
    
    return ( bytearray( [ 1, 0, 0, 1 ] ) )

   
    # ansk=dec_s_box(ansl)
    # ansl=dec_xor(ansk,key)
    # ansk=dec_subtract_last_two_first_two(ansl)
    # ansl=dec_swap_last_two(ansk)
    # ansk=dec_s_box(ansl)
    # ansppp=dec_xor(ansk,key)
    # return bytearray(ansppp)

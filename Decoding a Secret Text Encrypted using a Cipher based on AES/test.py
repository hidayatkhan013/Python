...
# 
#  YOU MUST ADD CODE TO MAKE THIS WORK.
...
def dec_decrypt( int_list, int_key ):
    ansx =dec_xor( dec_byte_to_nibbles( int_list ),dec_byte_to_nibbles( int_key ) )
    ans2 = dec_convert_nibbles_to_byte( ansx )
    print( "dec_xor \n" )
    print( format( ans2, '08b' ) )

    ans = \
        dec_swap_last_two( dec_byte_to_nibbles( ans2 ) )
    ans2 = dec_convert_nibbles_to_byte( ans )
    print( "dec_swap_last_two of " )
    print( format( ans2, '08b' ) )

    ans =dec_s_box( dec_byte_to_nibbles( ans2 ) )
    ans2 = dec_convert_nibbles_to_byte( ans )
    print( "dec_s_box of  " )
    print( format( ans2, '08b' ) )

    ansx =dec_xor( dec_byte_to_nibbles( ans2 ),dec_byte_to_nibbles( int_key ) )
    ans2 = dec_convert_nibbles_to_byte( ansx )
    print( " mid   dec_xor \n" )
    print( format( ans2, '08b' ) )
    ...
    ans = \
        dec_subtract_last_two_first_two( dec_byte_to_nibbles( ans2 ) )
    ans2 = dec_convert_nibbles_to_byte( ans )
    print( "dec_subtract_last_two_first_two \n" )
    print( format( ans2, '08b' ) )
    ...
    ans = \
        dec_swap_last_two( dec_byte_to_nibbles( ans2 ) )
    ans2 = dec_convert_nibbles_to_byte( ans )
    print( "mid dec_swap_last_two of " )
    print( format( ans2, '08b' ) )

    ans =dec_s_box( dec_byte_to_nibbles( ans2 ) )
    ans2 = dec_convert_nibbles_to_byte( ans )
    print( "las dec_s_box of  " )
    print( format( ans2, '08b' ) )
...
def dec_xor( nibbles, key ):
    ...
    # YOU MUST WRITE THIS FUNCTION
    ans=bytes(a ^ b for a, b in zip(nibbles, key))
...
def dec_swap_last_two( nibbles ):
    ...
    nibbles[2]=nibbles[3]
    nibbles[3]=temp 
    ans = bytearray(nibbles )
  #  ans=( (nibbles & 0x0F)<<4 | (nibbles & 0xF0)>>4 ) 
...
def dec_s_box( nibbles ):
   ...
   nibbles[0], nibbles[1] , nibbles[2], nibbles[3]= nibbles[2], nibbles[0] , nibbles[3], nibbles[1]
...
def dec_subtract_last_two_first_two( nibbles ):
    sub=int(nibbles[0])-int(nibbles[2])
    if(sub<0):  
        nibbles[0]=(-1*sub)
    else:
        nibbles[0]=sub
    sub=int(nibbles[1])-int(nibbles[3])
    if(sub<0):  
        nibbles[1]=(-1*sub)
    else:
        nibbles[1]=(sub)
...
if __name__ == '__main__':
    ...
    if len( sys.argv ) < 2:
        ...
        print( "dec_decrypt:" )
        dec_decrypt( 0b10000100, 0b01010100 )
